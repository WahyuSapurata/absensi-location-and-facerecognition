            ...
            <div class="card-body hover-scroll-overlay-y">
                <form class="form-data" enctype="multipart/form-data">
                    ...
                    <div class="mb-10">
                        ...
                        <select name="unit" class="form-select" data-control="select2" id="unit_select"
...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            control.overlay_form('Tambah', 'Pegawai');
        })
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/admin/add-dataguru', 'Tambah',
                    'Pegawai',
                    'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/admin/update-dataguru/' + uuid, 'Update',
                    'Pegawai', 'POST');
            }
        });
        ...
        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            let url = '/admin/show-dataguru/' + $(this).attr('data-uuid');
            control.overlay_form('Update', 'Pegawai', url);
        })

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/delete-dataguru/' + $(this).attr('data-uuid');
        ...
        let columns = [{
            ...
            render: function(data, type, row, meta) {
                let result =
                    `
                        <img src="{{ asset('storage/foto/${row.foto}') }}" style="max-width: 100%;" alt="no foto">
                    `;
                return result;
            }
        }, {
        ...
        let columnDefs = [{
            ...
            render: function(data, type, full, meta) {
                return `
                <a href="javascript:;" type="button" data-uuid="${data}" data-kt-drawer-show="true" data-kt-drawer-target="#side_form" class="btn btn-primary button-update btn-icon btn-sm ${full.role === "{{ $role }}" ? 'disabled-link' : ''}">
                ...
                <a href="javascript:;" type="button" data-uuid="${data}" data-label="Pegawai" class="btn btn-danger button-delete btn-icon btn-sm ${full.role === "{{ $role }}" ? 'disabled-link' : ''}">
        ...
        $(function() {
            control.push_select3(unit, '#unit_select');
            control.initDatatable('/admin/get-dataguru', columns, columnDefs);
        })
    </script>
@endsection
